...
<html>
  ...
  <s:if test="#session.riskinfo.bidper==1">
  <s:a href="bidinfo/changeState.action?id=%{#bidinfo.id}"  onClick="{if(confirm('确实要上标吗？')){return true;}return false;}" cssClass="title1" title="上标"></s:a>
  </s:if>
  <s:else>
   <s:a href="bidinfo/changeState.action?id=%{#bidinfo.id}" cssClass="title11" title="您没有权限进行此操作"></s:a>  
  </s:else>
      ...
      <s:a href="userinfo/findById.action?userinfo=%{#bidinfo.userid}" cssClass="title3" title="完善信息"></s:a>
       <s:a href="bidinfo/updateBid.action?id=%{#bidinfo.id}" cssClass="title2"> </s:a>
        <s:a href="bidinfo/delectBid.action?id=%{#bidinfo.id}" onClick="{if(confirm('确实要删除吗？')){return true;}return false;}" cssClass="title4">删除</s:a>
  ...
  <td>
  <s:a href= "bidinfo/offlineFinance.action?id=%{#bidinfo.id}" onClick="{if(confirm('确实要下线吗？')){return true;}return false;}">下线</s:a>
  </td>
  </s:if>
    <s:if test="%{#bidinfo.state=='已整合'}">
  <td><s:a href="bidinfo/changeState.action?id=%{#bidinfo.id}" cssClass="title11" title="已整合"></s:a>
      <s:a href="userinfo/findById.action?userinfo=%{#bidinfo.userid}" cssClass="title3" title="完善信息"></s:a>
       <s:a href="bidinfo/updateBid.action?id=%{#bidinfo.id}" cssClass="title2"> </s:a>
        <s:a href="bidinfo/delectBid.action?id=%{#bidinfo.id}" onClick="{if(confirm('确实要删除吗？')){return true;}return false;}" cssClass="title4">删除</s:a>
  </td>
...
<!--      <s:form id="form"  action="userinfo/findById.action">
<a href="bidinfo/addBid.action">新建标</a>
   <table border="0">
   <tr><input type="button" value="新建标" onclick="add()"/></tr>
